"""
CALCE battery dataset loader for Day 3.

CALCE files are often distributed as Excel workbooks or text/CSV exports.
This loader reads one file, normalizes its columns, and saves a processed CSV.
"""
...
def load_calce_file(path: str | Path, sheet_name: str | int | None = None, max_rows: int | None = None) -> pd.DataFrame:
    path = Path(path)
    ...
    df = add_basic_features(df, source="calce", file_path=path)

    available = [c for c in REQUIRED_AT_LEAST_ONE if c in df.columns and df[c].notna().any()]
    if not available:
        raise ValueError(
            f"CALCE file loaded but no useful battery columns were detected. "
