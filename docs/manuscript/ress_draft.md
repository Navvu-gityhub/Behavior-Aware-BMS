...
> 5. **Figures are drawn but not yet placed to a journal template.** Three
>    exist under `reports/figures/manuscript_fig*.png`, regenerated by
>    `scripts/generate_manuscript_figures.py`. Sizing, numbering and
>    captions still need RESS formatting.
...
![Figure 1](../../reports/figures/manuscript_fig1_loco_gap_distribution.png)

**Figure 1.** The LOCO gap across cohort draws on both derivations, matched
on k = 3, 4, 5. The interquartile box narrows by roughly threefold between
(a) and (b); the annotated noise-to-effect ratio does not move. Regenerate
with `python scripts/generate_manuscript_figures.py`.
...
![Figure 3](../../reports/figures/manuscript_fig3_target_derivations.png)

**Figure 3.** Cell CS2_5 under both derivations. (a) the `Cycle_Index`
frame alternates full and partial discharges, so no smooth function of
cycle number can track it; (b) the full-discharge frame admits only graded
full discharges.
...
![Figure 2](../../reports/figures/manuscript_fig2_per_cohort_coverage.png)

**Figure 2.** Per-cohort empirical coverage against the nominal guarantee
under leave-one-cohort-out. The aggregate meets nominal while individual
cohorts fall well below it, which is the argument for reporting worst-group
coverage rather than the median.
...
4. **Figures: drawn, not yet formatted.** All three candidates now exist and
   are regenerated from tracked artifacts by
   `scripts/generate_manuscript_figures.py`:
   (a) `manuscript_fig1_loco_gap_distribution.png` — the LOCO gap distribution
   across draws with both derivations overlaid, which is §5.2's whole argument
   in one panel; (b) `manuscript_fig2_per_cohort_coverage.png` — per-cohort
   conformal coverage against nominal (§5.5); (c)
   `manuscript_fig3_target_derivations.png` — the two target derivations for
   CS2_5 (§5.4).

   Figure 1 imports the recipes `tests/test_reported_numbers.py` uses to pin
   §5.2.1 and **refuses to render** if its recomputed IQR, effect and ratio
   disagree with the published values, so the figure and the prose cannot drift
   apart. What remains is journal formatting: column sizing, numbering and
   caption style to the RESS template.
